...
const { useNavigate, Link, useSearchParams } = rrdPkg;
import { useForm } from 'react-hook-form';
import { User, Mail, Phone, Lock, ShoppingCart, ShoppingBag, Box, Smartphone, Sparkles } from 'lucide-react';
      ...
      <div className="relative z-10 flex flex-col md:flex-row w-full md:max-w-[880px] min-h-screen md:min-h-[530px] bg-white rounded-none md:rounded-[20px] shadow-none md:shadow-[0_20px_50px_rgba(0,0,0,0.06)] border-0 md:border md:border-white/40 overflow-hidden transition-all duration-300">
        ...
        <section className="relative w-full md:w-[40%] bg-gradient-to-br from-[#E53935] via-[#FF4D4F] to-[#FF6B6B] text-white px-10 py-12 flex flex-col justify-between overflow-hidden">
          ...
          <div className="relative z-10 my-auto">
            <p className="text-xs md:text-sm font-semibold tracking-[0.2em] text-white/80 uppercase mb-4">JOIN US TODAY</p>
            ...
            <p className="text-sm md:text-base text-white/90 leading-relaxed font-light max-w-[280px]">
              Create an account to access your workspace and stay connected with your team.
            </p>
          </div>
          ...
          <div className="w-full max-w-sm mx-auto">
            ...
            <div className="text-center mb-6">
              <h2 className="text-xl md:text-2xl font-semibold text-[#1F2937] tracking-tight mb-2">
                {role === 'VENDOR' ? 'Create a Seller Account' :
                  role === 'TECHNICAL_SUPPORT' ? 'Create a Support Account' :
                    'Create an account'}
              </h2>
              <p className="text-[#6B7280] text-xs md:text-sm font-medium">
                {role === 'VENDOR' ? 'Register to start selling on IBCMart today.' :
                  role === 'TECHNICAL_SUPPORT' ? 'Register as a Technical Support Agent.' :
                    'Join the enterprise marketplace platform today.'}
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="relative">
                ...
                {errors.name && <p className="text-[#E53935] text-xs mt-1 pl-1 font-medium">{errors.name.message}</p>}
              </div>
              ...
              <div className="relative">
                <div className="relative group">
                  ...
                  <input
                    {...register('phone', { required: 'Mobile number is required', pattern: { value: /^[0-9]{10}$/, message: 'Mobile number must be exactly 10 digits' } })}
                    id="phone"
                    type="tel"
                    className="w-full border border-gray-200 rounded-[8px] pl-12 pr-4 py-3 text-sm font-medium text-gray-800 placeholder-gray-400 focus:outline-none focus:border-[#E53935]/60 focus:ring-2 focus:ring-[#E53935]/10 transition-all duration-200 bg-white"
                ...
                {errors.phone && <p className="text-[#E53935] text-xs mt-1 pl-1 font-medium">{errors.phone.message}</p>}
              </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-[#E53935] hover:bg-[#D32F2F] text-white font-semibold py-3.5 rounded-[8px] shadow-sm hover:scale-[1.005] active:scale-[0.995] transition-all duration-200 disabled:opacity-50 disabled:pointer-events-none text-sm cursor-pointer mt-4 flex items-center justify-center gap-2"
              >
            ...
            {/* Bottom Login Link */}
            <div className="mt-6 text-center text-xs md:text-sm font-medium">
